...
(function (global) {
  ...
  /** The environment GUID cannot be this value, whatever a page labelled it. */
  ...
  /**
   * Pull a usable environment GUID out of whatever was pasted.
   *
   * This box has to be forgiving, because there is no single place that
   * reliably shows the value. Copilot Studio's own Session details panel
   * prints "Environment ID: Default-{tenantId}", which is the environment's
   * NAME, so anyone following that label pastes something unusable. Rather
   * than argue with a label Microsoft wrote, accept anything that contains
   * the real GUID: a bare GUID, an Agents SDK connection string, a token
   * endpoint URL, or a block of copied text.
   */
   ...
   * looks usable.
   *
   * Every message here has to name a place that shows the RIGHT value. The
   * previous version sent people to Copilot Studio > Settings > Advanced >
   * Metadata, which is exactly where the misleading "Default-" alias is
   * printed, so following the advice reproduced the error. The Power Platform
   * admin centre is the one surface that shows the true GUID.
   */
  function describeEnvIdProblem(raw) {
    ...
    if (!id) {
      if (/^Default-/i.test(s)) {
        return "\u201CDefault-\u2026\u201D is the environment\u2019s name, not its ID, even though " +
          "Copilot Studio labels it \u201CEnvironment ID\u201D. The real ID is a plain GUID: open " +
          "admin.powerplatform.microsoft.com \u203A Manage \u203A Environments, click your " +
          "environment, and copy Environment ID from the Details panel.";
      }
      return "No environment ID found in that. Paste the GUID from " +
        "admin.powerplatform.microsoft.com \u203A Manage \u203A Environments \u203A your " +
        "environment \u203A Details, or paste the whole Agents SDK connection string and the " +
        "ID will be taken out of it.";
    }

    if (tenant && id === tenant) {
      return "That GUID is your tenant ID, not the environment ID. They are different values. " +
        "Get the environment ID from admin.powerplatform.microsoft.com \u203A Manage \u203A " +
        "Environments \u203A your environment \u203A Details.";
    }
