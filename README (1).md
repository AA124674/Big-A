...
**Schema name** from **Settings → Advanced → Metadata** and the environment's
actual **Environment ID**. If Metadata shows `Default-{tenant-id}`, that is the
environment name, not its Environment ID. Get the Environment ID from **Power
Platform admin center → Manage → Environments → your environment → Details**.
...
`Default-{tenant-id}` is an environment name, not the Environment ID. The GUID
after `Default-` is the tenant ID. When manual Agents SDK configuration is
necessary, get the Environment ID from **Power Platform admin center → Manage →
Environments → your environment → Details**. A complete Agents SDK connection
string is preferable because it avoids this ambiguity.
