...
(function (global) {
  ...
  function discoverTokenEndpoint(embedUrl, bearer, envId) {
    ...
    var defaultEnvHint = !envId && isDefaultAlias(environmentSegment(embedUrl))
      // Do NOT send people to Copilot Studio's own Metadata or Session details
      // panels for this. Both print "Environment ID: Default-{tenantId}",
      // which is the environment's NAME, so following that label reproduces
      // the failure. The Power Platform admin centre is the only surface that
      // shows the true GUID.
      ? " This agent is in the DEFAULT environment. Its URL shows \u201CDefault-\u201D followed by " +
        "your tenant ID, which is the environment's name rather than its ID, so the address " +
        "cannot be worked out from the URL alone. Get the Environment ID from " +
        "admin.powerplatform.microsoft.com \u203A Manage \u203A Environments \u203A your " +
        "environment \u203A Details, and paste it into Connection settings. Note that Copilot " +
        "Studio's own Metadata panel shows the \u201CDefault-\u2026\u201D name here, not the ID."
